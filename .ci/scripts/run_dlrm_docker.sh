...
# TODO remove sudo
sudo ssh -p "${DOCKER_SSH_PORT}" "${HEAD_NODE}" /opt/nvidia/torch-ucc/src/.ci/scripts/run_dlrm.sh ${TORCH_UCC_MODE} /opt/nvidia/torch-ucc/src/.ci/configs/$HOSTNAME/hostfile.txt
